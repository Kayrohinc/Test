...
public class ComplexEditor :EditorWindow
{
	...
	public static 	void Init ()
	{
		...
		if (window.parametrs == null) {
			...
			if (!Directory.Exists (Complex_Serializeable_Parametrs.ResourcesPath)) {
				Directory.CreateDirectory (Complex_Serializeable_Parametrs.ResourcesPath);
			}
			if (!Directory.Exists (Complex_Serializeable_Parametrs.StreamingPath)) {
				Directory.CreateDirectory (Complex_Serializeable_Parametrs.StreamingPath);
			}
		...
		for (int j = 0; j <parametrs.Count_of_Release; j++) { 
			...
			if (this.parametrs.Lister [j].Open) {
				...
				if (parametrs.Lister [j].Resource_Open) {
					GUILayout.Label ("Resources (Folders)");
					GUILayout.Space (10f);

					for (int i = 0; i < parametrs.Lister[j].Resource.Length; i++) {
						GUILayout.BeginHorizontal ();
						GUIStyle labelStyle = new GUIStyle (EditorStyles.label);
						parametrs.Lister [j].Resource [i] = EditorGUILayout.Toggle ("", parametrs.Lister [j].Resource [i]);
						var vt = labelStyle.CalcSize (new GUIContent (parametrs.Resources_path [i]));
						GUILayout.Label (parametrs.Resources_path [i], GUILayout.Width (vt.x));
						if (this.position.width - vt.x > 0)
							GUILayout.Label ("", GUILayout.Width (this.position.width - vt.x));
						GUILayout.EndHorizontal ();
					...
					if (GUILayout.Button (" Select All", GUILayout.Width (150))) {
						for (int i = 0; i < parametrs.Lister[j].Resource.Length; i++) {
							...
							parametrs.Lister [j].Resource [i] = true;
						}

					}
					if (GUILayout.Button (" Select None", GUILayout.Width (150))) {
						for (int i = 0; i < parametrs.Lister[j].Resource.Length; i++) {
							
							parametrs.Lister [j].Resource [i] = false;
						}
					}
					GUILayout.EndHorizontal ();
				}
				...
				if (parametrs.Lister [j].Streaming_Open) {
					GUILayout.Label ("Streaming Assets (Files)");
					GUILayout.Space (10f);

					for (int i = 0; i <parametrs.Streaming_path.Length; i++) {

						parametrs.Lister [j].Streaming [i] = EditorGUILayout.ToggleLeft (parametrs.Streaming_path [i], parametrs.Lister [j].Streaming [i], GUILayout.Width (400));

					}
					GUILayout.BeginHorizontal ();
					if (GUILayout.Button (" Select All", GUILayout.Width (150))) {
						for (int i = 0; i < parametrs.Lister[j].Streaming.Length; i++) {
							
							parametrs.Lister [j].Streaming [i] = true;
						}
					...
					if (GUILayout.Button (" Select None", GUILayout.Width (150))) {
						for (int i = 0; i < parametrs.Lister[j].Streaming.Length; i++) {
							...
							parametrs.Lister [j].Streaming [i] = false;
						}
					}
					GUILayout.EndHorizontal ();
				}
